...
| A self-feature-plus-neighbor operator has the planned $\kappa_\alpha$ expression. | Task 4 derivation and numerical moment tests. | Pending | Do not state as a completed contribution before Task 4 passes. |
| The exact expression matches empirical moments across a frozen parameter grid. | Task 7 per-seed synthetic artifacts and audited summary. | Pending | Do not claim direct empirical validation yet. |
