...
\begin{enumerate}
    ...
    \item[(A3)] \textbf{Dominated feature noise}: $\|\mu\|_{\Sigma^{-1}}^2 \mathrm{Var}(\sum_u w_{vu} Y_u) \ll 1 / k_{\mathrm{eff}}$, where $\|\mu\|_{\Sigma^{-1}}^2 := \mu^\top \Sigma^{-1} \mu$ is the Mahalanobis norm
\end{enumerate}
...
\end{align}
Under (A2), conditioning on both $Y_v$ and the neighborhood structure $\mathcal{N}(v)$, the cross terms $\mathrm{Cov}(\sum_u w_{vu} Y_u \mu, \sum_u w_{vu} \varepsilon_u \mid Y_v, \mathcal{N}(v))$ vanish because $\varepsilon_u$ is independent of $Y_u$ by the generative model (Definition~\ref{def:sparse_csbm}). Taking expectation over $\mathcal{N}(v)$ introduces correlations only through shared neighbors, which occur with probability $O(k^2/n)$ in the sparse regime. Specifically, for distinct neighbors $u, u' \in \mathcal{N}(v)$:
\begin{equation}
    \mathbb{E}[\varepsilon_u \varepsilon_{u'}^\top \mid Y_v, u \neq u'] = \mathbb{P}(u, u' \text{ share edge} \mid \mathcal{N}(v)) \cdot O(\Sigma) = O(k/n) \cdot \Sigma
\end{equation}
Thus the cross-term contribution is $O(k^2/n) \cdot \|\mu\| \|\Sigma\|^{1/2}$, which vanishes as $n \to \infty$ with $k = O(1)$.

The label mixing variance satisfies:
\begin{equation}
    \mathrm{Var}\left(\sum_u w_{vu} Y_u \,\Big|\, Y_v\right) = \sum_u w_{vu}^2 (1 - \rho^2) + \underbrace{\sum_{u \neq u'} w_{vu} w_{vu'} \mathrm{Cov}(Y_u, Y_{u'} \mid Y_v)}_{\text{correlation terms}}
\end{equation}
where the correlation terms satisfy $|\mathrm{Cov}(Y_u, Y_{u'} \mid Y_v)| \leq \rho^2$ and sum to $O(\rho^2)$ for tree-like neighborhoods, yielding total correlation contribution $O(\rho^2/k_{\mathrm{eff}})$ which is subdominant.
...
\begin{remark}[On Condition (A3): When Does Feature Noise Dominate?]
\label{rem:a3_condition}
Condition (A3) requires $\|\mu\|_{\Sigma^{-1}}^2 \mathrm{Var}(\sum_u w_{vu} Y_u) \ll 1 / k_{\mathrm{eff}}$, equivalently $\mathcal{D}(X) (1-\rho^2) / k_{\mathrm{eff}} \ll 1$. This is satisfied when:
\begin{enumerate}
    \item \textbf{Low feature separability}: $\mathcal{D}(X) = 4\mu^\top \Sigma^{-1} \mu \ll k_{\mathrm{eff}}$ (features are noisy relative to class separation)
    \item \textbf{High degree}: Large $k_{\mathrm{eff}}$ makes the correction factor $1/(1 + \mathcal{D}(X)(1-\rho^2)/k_{\mathrm{eff}})$ close to 1
    \item \textbf{High edge correlation}: When $|\rho| \to 1$, label mixing variance $(1-\rho^2) \to 0$
\end{enumerate}

\textbf{When (A3) fails}: For datasets with high feature separability ($\mathcal{D}(X) \gg k_{\mathrm{eff}}$) and moderate $\rho$, label mixing noise may be comparable to feature noise. In this regime, the simplified formula $\kappa = \rho^2 k$ becomes an \textit{optimistic estimate} (upper bound on the true SNR gain), because label mixing variance contributes additional noise in the denominator. The corrected discriminability satisfies:
\begin{equation}
    \frac{\mathcal{D}(A)}{\mathcal{D}(X)} \leq \frac{\rho^2 k_{\mathrm{eff}}}{1 + \mathcal{D}(X)(1-\rho^2)/k_{\mathrm{eff}}} \leq \rho^2 k_{\mathrm{eff}} = \kappa
\end{equation}
The correction factor $1/(1 + \mathcal{D}(X)(1-\rho^2)/k_{\mathrm{eff}})$ is close to 1 when either (i) $\mathcal{D}(X)$ is small (low feature separability), (ii) $|\rho|$ is close to 1 (high edge correlation), or (iii) $k_{\mathrm{eff}}$ is large (high degree). Our empirical validation includes high-separability datasets (CSBM with MLP $\approx 99\%$), and the predictions remain accurate, suggesting the approximation is robust in practice.
\end{remark}
...
\input{sections/04_threshold_theory}
...
We provide a rigorous derivation following the belief propagation framework of~\cite{abbe2018community}.

\textbf{Step 1 (Message structure):} On a tree, each message $m_{u \to v}^{(t)}$ depends only on the subtree rooted at $u$ excluding the branch toward $v$. By induction on $t$:
\begin{equation}
    m_{u \to v}^{(t)} = \sum_{w \in T_u^{(t)} \setminus \{v\}} X_w \cdot c_{uw}^{(t)}
\end{equation}
where $T_u^{(t)}$ is the $t$-hop subtree from $u$ and $c_{uw}^{(t)}$ are combinatorial coefficients counting paths.

\textbf{Step 2 (Signal computation):} Taking conditional expectation:
\begin{align}
    \mathbb{E}[m_{u \to v}^{(t)} \mid Y_v = y] &= \sum_{w \in T_u^{(t)}} c_{uw}^{(t)} \cdot \mathbb{E}[X_w \mid Y_v = y] \\
    &= \sum_{w \in T_u^{(t)}} c_{uw}^{(t)} \cdot \rho^{d(w,v)} \mu \cdot y
\end{align}
where $d(w,v)$ is the graph distance. On a $(k-1)$-ary tree, there are $(k-1)^{t-j}$ nodes at distance $j$ from the root within the $t$-hop neighborhood, and summing over paths yields total signal proportional to $((k-1)\rho)^t$.

\textbf{Step 3 (Noise computation):} The variance is:
\begin{equation}
    \mathrm{Var}(m_{u \to v}^{(t)} \mid Y_v) = \sum_{w \in T_u^{(t)}} (c_{uw}^{(t)})^2 \Sigma + \text{(label mixing)} + \text{(cross-correlation)}
\end{equation}
For sum aggregation with unit coefficients, the feature noise contribution grows as $|T_u^{(t)}| \cdot \Sigma \propto (k-1)^t \Sigma$. Cross-correlations vanish on trees (no shared ancestors within $T_u^{(t)}$).

\textbf{Step 4 (Discriminability ratio):} Combining steps 2--3:
\begin{equation}
    \mathcal{D}^{(t)} = \frac{\|\Delta^{(t)}\|_{\Sigma^{-1}}^2}{\text{noise}} \propto \frac{((k-1)\rho)^{2t}}{(k-1)^t} = ((k-1)\rho^2)^t
\end{equation}
The multiplicative structure $\mathcal{D}^{(t)} = ((k-1)\rho^2)^t \mathcal{D}^{(0)}$ follows, with the base case $\mathcal{D}^{(0)} = \mathcal{D}(X)$.
\end{proof}
...
\input{sections/04_csbm_real_bridge}
...
\begin{remark}[Extension to Multi-Class Classification ($C > 2$)]
\label{rem:multiclass_extension}
Our theoretical analysis is derived for binary classification (CSBM with $C = 2$), yet our experiments include datasets with $C \in \{3, 7, 40\}$ classes. We clarify the scope and empirical justification for this extension.

\textbf{(a) Theoretical scope:}
The SNR discriminability $\mathcal{D}(Z) = \|\mu\|^2 / \sigma^2$ and its multiplicative property under aggregation ($\mathcal{D}(A) = \rho^2 k \mathcal{D}(X)$) are derived under:
\begin{itemize}
    \item Binary labels $Y \in \{-1, +1\}$ with equal priors
    \item Gaussian class-conditionals with shared covariance
\end{itemize}
For $C > 2$, the analysis requires replacing scalar $\rho$ with a class confusion matrix.

\textbf{(b) Multi-class effective correlation:}
For $C$-class problems with homophily $h$, the \textbf{effective edge correlation} becomes:
\begin{equation}
    \rho_C = \frac{Ch - 1}{C - 1}
\end{equation}
This reduces to $\rho_2 = 2h - 1$ for binary. For Cora ($C = 7$, $h = 0.81$): $\rho_7 = (7 \times 0.81 - 1)/(7-1) = 0.78$.

\textbf{(c) Empirical justification:}
Despite the binary theoretical framework, our predictions hold on multi-class datasets because:
\begin{enumerate}
    \item The \textit{qualitative} threshold behavior ($\kappa \gtrless 1$ determining aggregation benefit) extends to multi-class via $\rho_C$
    \item The Information Budget ceiling $\mathcal{B} = 1 - \text{Acc}_{\text{MLP}}$ is definitionally class-agnostic
    \item Empirically, all 23 datasets (including $C = 40$ for ogbn-arxiv) satisfy our predictions
\end{enumerate}

\textbf{(d) Practical sufficiency:}
While a rigorous multi-class SNR theory analyzing the full $C \times C$ label transition matrix would provide additional theoretical grounding, our empirical results demonstrate that the binary-derived insights extend effectively to multi-class settings. The framework's practical utility is validated across all tested scenarios.
\end{remark}
