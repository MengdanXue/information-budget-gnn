...
\textbf{Heterophily-aware methods}: When $h$ is low, standard GNNs fail. H2GCN~\cite{zhu2020beyond} uses 2-hop neighbors and ego-neighbor separation. LINKX~\cite{luan2022linkx} separates features from structure entirely. MixHop~\cite{abu2019mixhop} and GPR-GNN~\cite{chien2021gprgnn} use multi-hop aggregation with learnable coefficients. FAGCN~\cite{fagcn2021} introduces frequency-adaptive filtering to handle both homophily and heterophily. GloGNN~\cite{li2022glognn} discovers global homophily patterns even in locally heterophilous graphs. ACM-GCN~\cite{luan2022acmgcn} uses adaptive cross-hop message passing. Our Dual Heterophily framework (Section~\ref{sec:heterophily}) explains \textit{when} these methods help: Type A (recoverable via 2-hop) vs Type B (irrecoverable, MLP optimal).
...
\textbf{Aggregation analysis}: Cai and Wang~\cite{cai2020note} showed mean aggregation has limitations. Hamilton~\cite{hamilton2020book} compared aggregators extensively. Our Proposition~\ref{prop:concat_bound} provides theoretical grounding for concatenation's superiority.
...
\begin{enumerate}
    ...
    \item \textbf{Heterophily typology}: Rather than treating low-$h$ as a single regime, we distinguish Type A (where H2GCN helps) from Type B (where MLP is optimal).
\end{enumerate}
