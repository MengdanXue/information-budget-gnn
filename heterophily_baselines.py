...
class MixHop(nn.Module):
    ...
    Mixes 0-hop, 1-hop, and 2-hop neighborhood information.
    """
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2, dropout=0.5):
        super().__init__()
        self.dropout = dropout
        ...
        # Each hop produces hidden_channels // 3 features, total = hidden_channels
        hop_dim = hidden_channels // 3
        actual_hidden = hop_dim * 3  # Ensure divisibility
        ...
        # Layer 2: hidden -> output (directly, no second MixHop layer for simplicity)
    ...
    def forward(self, x, edge_index):
        ...
        # 1-hop (A @ x)
        ax = self._propagate(x, edge_index_with_self, norm)
        h1 = self.lin1_1(ax)

        # 2-hop (A^2 @ x)
        a2x = self._propagate(ax, edge_index_with_self, norm)
        h2 = self.lin2_1(a2x)

        # Concatenate and apply non-linearity
    ...
    def _propagate(self, x, edge_index, norm):
        """Simple message passing: normalized adjacency @ x"""
        row, col = edge_index
